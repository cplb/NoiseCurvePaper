...
An alternative characterisation of the signal amplitude commonly used for burst sources is the root-sum-square of the waveform polarisations, $h_\mathrm{rss}$. If $\tilde{h}(f)$ is constant across the bandwidth $\Delta f$ this is related to the characteristic strain via Equation \ref{eq:rela}. In this work, we favour a constant $h_\mathrm{c}(f)$ rather than $h_\mathrm{rss}$ for consistency with the other types of source.
\begin{eqnarray}
