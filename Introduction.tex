...
When integrated over all positive frequencies the PSD gives the mean square noise amplitude. Starting with taking the average of the square of the detector noise:
...
\end{eqnarray}
Using the property of Fourier transforms, ${\cal{F}}\left\{ n(t+\tau) \right\}(f)=\tilde{n}(f)\exp (-2\pi \rmi f\tau )$, the time average can be turned into an ensemble average and hence the definition of PSD in (\ref{eq:psd}) may be used.
\begin{eqnarray}\label{eq:meansquare1}
