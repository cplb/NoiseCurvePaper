...
When integrated over all positive frequencies the power spectral density gives the mean square noise amplitude. Starting with taking the \emph{time} average of the square of the detector noise:
\begin{eqnarray}\label{eq:meansquare}
\overline{n(t)^{2}} &= \lim_{T\rightarrow\infty}\frac{1}{2T}\int_{-T}^{T}\mathrm{d}t\; n(t) ^{2} \nonumber \\
 &= \lim_{T\rightarrow\infty}\frac{1}{2T}\int_{-T}^{T}\mathrm{d}t\; n(t)n^{*}(t) \nonumber \\
 &= \lim_{T\rightarrow\infty}\frac{1}{2T}\int_{-T}^{T}\mathrm{d}t\; \int_{-\infty}^{\infty}\mathrm{d}f\;\int_{-\infty}^{\infty}\mathrm{d}f'\;\tilde{n}(f)\tilde{n}^{*}(f')e^{2\pi \rmi ft }e^{-2\pi \rmi f't } \nonumber\\
 &= \lim_{T\rightarrow\infty}\frac{1}{2T}\int_{-T}^{T}\mathrm{d}t\; \int_{-\infty}^{\infty}\mathrm{d}f\;\int_{-\infty}^{\infty}\mathrm{d}f'\;{\cal{F}}\left\{ n(\tau) \right\}(f){\cal{F}}\left\{ n(\tau) \right\}(f')^{*}e^{2\pi \rmi ft }e^{-2\pi \rmi f't }
\end{eqnarray}
Where in the penultimate line we have used the definition of the Fourier transform, (\ref{eq:defFourier}), and in the last line we have just changed notation. A property of Fourier transforms is that in the time domain a translation by amount $t$ translates to a rotation in phase by $2\pi ft$ in the frequency domain; if ${\mathcal{F}}\left\{ n(\tau) \right\} = \tilde{n}(f)$ then ${\mathcal{F}}\left\{ n(\tau-t) \right\} = \tilde{n}(f)\exp (2\pi \rmi ft )$. Therefore the exponential factors in (\ref{eq:meansquare}) may be absorbed as a time-translation of the noise by an amount $t$
\begin{eqnarray}
...
\end{eqnarray}
Using the ergodic property of the system described above the time average (denoted by $\overline{(\ldots)}$) may now be changed into an ensemble average (denoted by $\left<\ldots\right>$).
The noise is stationary so it's expectation value is unchanged by the time translation performed above. Therefore using (\ref{eq:psd}) gives
...
\end{equation}
where we have introduced the inner product between signal $A$ and $B$ as \citep{Finn1992}
