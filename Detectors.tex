...
\begin{figure}
 ...
 \caption{The angular response function of an interferometric detector shown both as a surface plot and in an Aitoff--Hammer projection. The quentity that is plotted is is the polarisation average, $\int\textrm{d}\psi \;((F^{+})^{2}+(F^{\times})^{2})/2$ \cplb{Should this be divided by $2\pi$ too?}. The response is a function of two sky angles, $\theta$ and $\phi$, and varies between $0$ and $1$. The two detector arms lie in the $x$--$y$ plane either side of one of the zeros in the response.}
 \label{fig:LIGO}
\end{figure}
